...
■カツミ（60代前半・女性）
- 役割：年金の専門家、解説役
- 性格：落ち着いていて優しく丁寧、視聴者に寄り添う
- 話し方：標準語（関西弁は使わない）
- 隠れ設定（たまーーーに出す程度、10回に1回くらい）：
  ・韓国ドラマ沼（ドラマの話で急に熱くなる）
  ・温泉マニア（旅行・温泉の話で急に詳しくなる）
  ・甘いもの大好き（スイーツの話で目が輝く）
  ・瀬戸内寂聴さんのファン（たまに「寂聴さんも言ってたけど…」）

■ヒロシ（40代前半・男性）
- 役割：視聴者代弁、素朴な疑問を聞く
- 立場：親世代のために年金を勉強中
- 性格：ちょっとお馬鹿でのんびり、リアクション大きめ
- 話し方：標準語（関西弁は使わない）
- 隠れ設定（たまーーーに出す程度、10回に1回くらい）：
  ・涙もろい（感動話でウルっとくる）
  ・映画マニア（古い映画の引用がたまに出る）
  ・昭和のアイドルが好き（中森明菜、小泉今日子、松田聖子）
    → たまに「明菜ちゃんの歌でさ…」みたいに出す
  ・ひろさちや（仏教思想家）のファン
    → たまに「ひろさちやさんが言ってたけど…」みたいに出す

■二人の関係性
- ※夫婦じゃない！「お母さんと息子」みたいな関係
- カツミが教えて、ヒロシが素直に聞く
...
【トーンと姿勢】
- カツミは優しく丁寧に解説
- ヒロシは視聴者目線で素朴な疑問をぶつける
- 視聴者に寄り添う温かい雰囲気

【会話の特徴】
- カツミ「ヒロシさん、今日は年金の大事なお話をしましょうね」
- ヒロシ「はい！親のためにしっかり勉強します」
- カツミ「その気持ち、とても大事よ。じゃあ順番に見ていきましょう」
- ヒロシ「え、そうなんですか？知らなかった…」
- カツミ「大丈夫、今日知れたからよかったのよ」
...
【トークの流れ例】
カツミ「ヒロシさん、今日は年金の繰り下げ受給についてお話ししましょう」
ヒロシ「繰り下げ受給って、70歳まで待つと増えるやつですよね？」
カツミ「そうそう。親御さんの年金を考える時に大事なポイントなの」
ヒロシ「うーん、正直よくわかってないんですよね…」
カツミ「大丈夫よ、順番に見ていきましょうね」
〜解説〜
カツミ「ヒロシさん、親御さんのねんきん定期便、見たことある？」
ヒロシ「あ、そういえば実家の引き出しにあったかも…」
カツミ「まずはそこからね。皆さんも、ご両親の定期便を一緒に確認してみてくださいね」
...
def generate_summary_table_image(script: dict, output_path: str):
    ...
    try:
        ...
        rank_font = ImageFont.truetype(font_path, 52)
        item_font = ImageFont.truetype(font_path, 28)
    except:
        try:
    ...
    frame_padding = 25
    frame_width = title_width + frame_padding * 2 + 60
    ...
    draw.rounded_rectangle(
        [(frame_x - 5, frame_y - 5), (frame_x + frame_width + 5, frame_y + frame_height + 5)],
        radius=15,
        fill='#FFD700'
    )
    # 内側（ダーク）
    ...
    title_y = frame_y + frame_padding - 3
    # 影
    ...
    for i, ranking in enumerate(sorted_rankings[:5]):
        ...
        item_title = ranking.get("title", "")

        y = card_start_y + i * (card_height + card_spacing)
        colors = bar_colors.get(rank, ('#4169E1', '#87CEEB'))

        # カード影
        draw.rounded_rectangle(
            [(card_margin + 5, y + 5), (card_margin + card_width + 5, y + card_height + 5)],
            radius=15,
            fill='#000000'
        )
        ...
        # カード上部のハイライト
        draw.rounded_rectangle(
            [(card_margin + 3, y + 3), (card_margin + card_width - 3, y + 25)],
            radius=12,
            fill=(60, 60, 90)
        )
        ...
        rank_x = card_margin + 30
        rank_y = y + (card_height - (rank_bbox[3] - rank_bbox[1])) // 2 - 5

        # 順位の影
        draw.text((rank_x + 3, rank_y + 3), rank_str, fill='#000000', font=rank_font)
        # 順位本体（順位色）
        draw.text((rank_x, rank_y), rank_str, fill=colors[0], font=rank_font)

        # 「位」
        pos_bbox = draw.textbbox((0, 0), "位", font=item_font)
        draw.text((rank_x + rank_w + 5, rank_y + 20), "位", fill='#FFFFFF', font=item_font)

        # ===== バーグラフ（中央） =====
        bar_x = card_margin + 130
        bar_y_pos = y + 58
        bar_height = 28
        ...
        # バー上部の光沢
        for j in range(bar_height // 3):
            alpha = 80 - j * 10
            if alpha > 0:
                draw.line(
                    [(bar_x + 2, bar_y_pos + j), (bar_x + bar_width - 2, bar_y_pos + j)],
                    fill=(255, 255, 255)
                )
        ...
        # タイトル影
        draw.text((title_x + 1, title_y + 1), display_title, fill='#000000', font=item_font)
        # タイトル本体
        draw.text((title_x, title_y), display_title, fill='#FFFFFF', font=item_font)
