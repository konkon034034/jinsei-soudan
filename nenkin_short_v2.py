...
- 話し方：標準語（関西弁は使わない）
- 視聴者を仲間に引き込む：「ここだけの話」「知っておいてほしいの」感
...
【会話スタイル】
- カツミ「ヒロシさん、ここだけの話なんだけど…」
- ヒロシ「え、なんですか？」
- カツミ「この表、知らない人結構多いのよ」
- ヒロシ「えっ、そうなんですか？僕も知らなかった…」
- カツミ「でしょ？親御さんにも教えてあげてね」
- ヒロシ「はい！これは保存しておきます」
...
def extract_facts_from_script_short(script: list) -> list:
...
def gemini_fact_check_short(script: list, table_data: dict, key_manager) -> dict:
    """Geminiによるファクトチェック（ショート用）"""
    ...
    prompt = f"""
【重要】以下の年金ショート動画の台本を厳密にファクトチェックしてください。

確認項目:
1. 金額（〇〇円、〇〇万円）は元のデータと一致しているか？
...
少しでも怪しい情報、元データと異なる数字があれば指摘してください。
...
def web_search_fact_check_short(script: list, key_manager) -> dict:
    """Web検索で数字・日付を裏取り（ショート用）"""
    api_key = key_manager.get_working_key()
    if not api_key:
        return {"has_error": False, "errors": [], "message": "APIキーなし（スキップ）"}
    ...
    # 台本から事実を抽出
    facts = extract_facts_from_script_short(script)
    if not facts:
        return {"has_error": False, "errors": [], "message": "検証対象なし"}

    # 重要な事実のみ検証（最大3件）
    important_facts = facts[:3]
...
def claude_fact_check_short(script: list, table_data: dict) -> dict:
    """Claude APIによるクロスチェック（ショート用）"""
...
def fix_script_errors_short(script: list, errors: list, key_manager) -> list:
    """エラーを修正した台本を生成（ショート用）"""
    api_key = key_manager.get_working_key()
    if not api_key:
        return script
    ...
    fix_prompt = f"""
以下の年金ショート動画台本にエラーが見つかりました。修正してください。
...
- 修正後の台本全文をJSON配列形式で出力してください
- 元の台本と同じ構造（speakerとtextのリスト）を維持してください
"""
...
def triple_fact_check_short(script: list, table_data: dict, key_manager) -> list:
    """3重ファクトチェック - 1つでもNGなら修正（ショート用）"""
    ...
    for attempt in range(max_retries):
        ...
        gemini_result = gemini_fact_check_short(script, table_data, key_manager)
        ...
        # Step 2: Web検索で裏取り
        print("    [2/3] Web検索チェック...")
        web_result = web_search_fact_check_short(script, key_manager)
        ...
        claude_result = claude_fact_check_short(script, table_data)
        ...
        script = fix_script_errors_short(script, all_errors, key_manager)
...
def main():
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # STEP4.3: 3重ファクトチェック
            print("\n[4.3/7] 3重ファクトチェック実行中...")
            script = triple_fact_check_short(script, table_data, key_manager)
